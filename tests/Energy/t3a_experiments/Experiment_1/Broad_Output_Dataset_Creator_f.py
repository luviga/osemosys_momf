...
df_output.to_csv ( 'OSEMOSYS_GUA_Energy_Output.csv', index = None, header=True)
df_output.to_csv ( 'OSEMOSYS_GUA_Energy_Output_' + str( today ).replace( '-', '_' ) + '.csv', index = None, header=True)
#
df_input = dfa_list[1]
df_input.to_csv ( 'OSEMOSYS_GUA_Energy_Input.csv', index = None, header=True)
df_input.to_csv ( 'OSEMOSYS_GUA_Energy_Input_' + str( today ).replace( '-', '_' ) + '.csv', index = None, header=True)
